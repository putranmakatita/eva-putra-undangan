...
const fotoInject = (folder, listData, idCont, idIndic, dataTarget) => {
    ...
    for (let i = 0; i < listData.length; i++) {
        ...
        let curr = `<div class="carousel-item ${active}">
                    <img src="${folder}${listData[i]}" class="d-block w-100" onclick="modalFoto(this)">
                </div>`;
...
window.addEventListener('load', () => {
    ...
    let listFotoHeader = [];
    let listFotoDetail = [];
    let folder = "https://web-services.rcelectronic.co.id/assets/Eve _ Putra/Edited Pilihan/";

    for (let i = 29; i <= 48; i++) {
        let imgName = `IMG_31${i}.JPG`;
        if (i <= 31) {
            listFotoHeader.push(imgName);
        } else {
            listFotoDetail.push(imgName);
        }
    }
    // header
